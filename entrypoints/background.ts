...
export default defineBackground(() => {
	...
	browser.alarms.onAlarm.addListener(async (alarm) => {
		const existingTabId = (await browser.storage.session.get("activeTabId"))
			?.activeTabId as number | undefined;
		...
		if (liveGames.success) {
			const liveGame = liveGames.output.data.esports.events.find(
				(e) => e.match?.id === matchId
			);
			if (liveGame != null) {
